...
class SVD():
    ...
    def _preprocess_data(self, X, train=True):
        ...
        if train:
            u_ids = np.unique(X[:,0]).tolist()
            i_ids = np.unique(X[:,1]).tolist()
    ...
    def fit(self, X, X_val=None, early_stopping=False, shuffle=False):
        ...
        ### build unique i/i list    
        self.user_uids = np.unique(X[:,0]).tolist()
        self.item_uids = np.unique(X[:,1]).tolist()
    ...
    def predict_pair(self, u_id, i_id, clip=True):
        ...
        if u_id in self.user_uids:
            user_known = True
            pred += self.bu[u_id]

        if i_id in self.item_uids:
            item_known = True
            pred += self.bi[i_id]
    ...
    def _on_epoch_begin(self, epoch_ix):
        ...
        if epoch_ix%100 == 0:
            print('Epoch {}/{}'.format(epoch_ix + 1, self.n_epochs))
